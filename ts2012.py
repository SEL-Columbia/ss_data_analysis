...
def range_query():
   ...
   conn=sqlite3.connect('/home/apan/ml00_0_2012.db')
   ...
   for (ts,w,i)  in c.execute("select timestamp,watthourssc20,id from ml00_0_2012  order by timestamp "):
   ...
   c.executemany('insert into minute_resolution2_cdf values(?,?)',d)
   conn.commit()
