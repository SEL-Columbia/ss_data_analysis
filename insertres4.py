...
def calcWH(cdf,tsArr):
    ...
    if resetCount>=2:
       print "multiple resets detected in one minute interval . resetCount="+str(resetCount)
       return -1
    else:
       return _sum
      ...
      for _file in filenames:
         ...
         if _file.find("_ts.db")==-1:
           ...
           if site=="ug03" or site=="ug07":
             print "Updating correction sites....."+site
             try:
            #if site[0:2]=='ug':
               ...
               for (ts,w)  in c.execute(cmd):  
                    ...
                    if curr==prev:
                      ...
                      if  prev!="YYYY-mm-dd HH:MM:SS" and  curr[0:16]!=prev[0:16]:
                         print "minute boundary"
                      else:
                         v[ts]=w

                    if  prev!="YYYY-mm-dd HH:MM:SS" and  curr[0:16]!=prev[0:16]:# minutely wattage resolution
                         cdf=[]   
                         tsArr=[]
                         ...
                         v[ts]=w
                         if totalwh!=-1 :
                           d.append((prev[0:16]+":00",totalwh,))
                    prev=curr



               c.executemany('insert into minute_res_watthours values(?,?)',d)
