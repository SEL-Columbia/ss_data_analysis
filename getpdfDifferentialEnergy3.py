    ...
    for basedir in ['apan','apan2','apan3']:
      for year in ['2010',  '2011','2012','2013']:
        ...
        try:
           ...
           #for (ts,w) in c.execute("select ts,wh  from hour_res_watthours  where  ts like '% 18:%' or ts like '% 19:%' or ts like '% 20:%' or ts like  '% 21:%' or ts like '% 22:%' or ts like '% 23:%'"):
           #  v[ts]=w
...
#                diff.append(v[ts]-prevwh)
#                if (v[ts]-prevwh >=-1) and ( v[ts]-prevwh<=1):
#                   print "time="+str(ts)+"current wh="+str(v[ts])+"previous="+str(prevwh)
    ...
    ax.set_xlabel('change in watt hours from hour to hour')
